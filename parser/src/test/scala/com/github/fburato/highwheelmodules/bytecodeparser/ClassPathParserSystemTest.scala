...
import com.example.classliterals.{
  HasFieldOfTypeClassFoo,
  MethodAccessFooClassLiteral,
  StoresFooArrayClassLiteralAsField,
  StoresFooClassLiteralAsField
}
import com.example.generics.{
  BoundedByFoo,
  HasCollectionOfFooParameter,
  ImplementsGenericisedInterface,
  ReturnsCollectionOfFoo
}
...
class ClassPathParserSystemTest
    extends AnyWordSpec
    with Matchers
    with MockitoSugar
  ...
  def onlyExampleDotCom: ClassParser = new ClassPathParser(el =>
    el.asJavaName.startsWith("com.example")
  )
  ...
  def checkApply(
    ...
    verify(accessVisitor)
      .apply(AccessPoint(ElementName.fromClass(source), accessPointName), accessType(dest), aType)
  ...
  def checkApply(
    ...
    verify(accessVisitor)
      .apply(accessType(source), AccessPoint(ElementName.fromClass(dest), accessPointName), aType)

  def checkApply(
    source: Class[_],
    sourceAccessPointName: AccessPointName,
    dest: Class[_],
    destAccessPointName: AccessPointName,
    aType: AccessType
  ): Unit =
    verify(accessVisitor).apply(
      AccessPoint(ElementName.fromClass(source), sourceAccessPointName),
      AccessPoint(ElementName.fromClass(dest), destAccessPointName),
      aType
    )
  ...
  "parse" should {
    ...
    "detect signature dependency when class returns an array" in {
      ...
      checkApply(
        classOf[ReturnsArrayOfFoo],
        method("foo", classOf[Array[Foo]]),
        classOf[Foo],
        SIGNATURE
      )
    }
    ...
    "detect signature dependency when method has parameter of another type" in {
      ...
      checkApply(
        classOf[HasFooAsParameter],
        methodWithParameter("foo", classOf[Foo]),
        classOf[Foo],
        SIGNATURE
      )
    }
    ...
    "detect signature dependency when method has parameter of array of another type" in {
      ...
      checkApply(
        classOf[HasFooArrayAsParameter],
        methodWithParameter("foo", classOf[Array[Foo]]),
        classOf[Foo],
        SIGNATURE
      )
    }
    ...
    "detect uses dependency when method code uses a type via constructor" in {
      ...
      checkApply(
        classOf[ConstructsAFoo],
        method("foo", classOf[Object]),
        classOf[Foo],
        method("(init)", "()V"),
        USES
      )
    ...
    "detect uses dependency when method code calls a method of a type" in {
      ...
      checkApply(
        classOf[CallsFooMethod],
        method("foo", classOf[Object]),
        classOf[Foo],
        method("aMethod", classOf[Object]),
        USES
      )
    }
    ...
    "detect annotation dependency when type annotated at method level" in {
      ...
      checkApply(
        classOf[AnnotatedAtMethodLevel],
        method("foo", "()V"),
        classOf[AnAnnotation],
        ANNOTATED
      )
    }
    ...
    "detect annotation dependency when type annotated at parameter level" in {
      ...
      checkApply(
        classOf[AnnotatedAtParameterLevel],
        method("foo", "(I)V"),
        classOf[AnAnnotation],
        ANNOTATED
      )
    ...
    "not detect annotation dependency when type annotated at variable level" in {
      ...
      verify(accessVisitor, never).apply(
        accessType(classOf[AnnotatedAtVariableLevel]),
        accessType(classOf[AnAnnotation]),
        ANNOTATED
      )
    }
    ...
    "detect uses dependency when nested type calls parent class method" in {
      ...
      checkApply(
        classOf[CallsMethodFromFooWithinInnerClass],
        method("foo", "()V"),
        classOf[Foo],
        method("aMethod", classOf[Object]),
        USES
    ...
    "detect uses dependency when type stores class array literal in method" in {
      ...
      checkApply(
        classOf[StoresFooArrayClassLiteralAsField],
        method("(init)", "()V"),
        classOf[Foo],
        USES
      )
    ...
    "detect uses dependency when type uses class literal in method" in {
      ...
      checkApply(
        classOf[MethodAccessFooClassLiteral],
        method("foo", classOf[Class[_]]),
        classOf[Foo],
        USES
      )
    }
    ...
    "detect unconnected methods" in {
      ...
      verify(accessVisitor).newAccessPoint(
        AccessPoint(ElementName.fromClass(classOf[Foo]), method("aMethod", "()Ljava/lang/Object;"))
      )
    }
    ...
    "detect uses dependency when method instantiates an anonymous interface instance" in {
      ...
      checkApply(
        classOf[UsesAnInterfaceInMethod],
        method("foo", "()V"),
        classOf[AnInterface],
    ...
    "detect uses dependency when type uses a method reference" in {
      ...
      checkApply(
        classOf[UsesMethodReference],
        method("foo", "()V"),
        classOf[Foo],
        method("aMethod", "()Ljava/lang/Object;"),
        USES
      )
    }
  }
