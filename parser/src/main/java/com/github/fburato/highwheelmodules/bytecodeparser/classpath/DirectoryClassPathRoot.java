...
public class DirectoryClassPathRoot implements ClasspathRoot {
    ...
    public InputStream getData(final ElementName classname) throws IOException {
        return getResource(classname.asJavaName().replace('.', File.separatorChar).concat(".class"));
    }
    ...
    private ElementName fileToClassName(final File f) {
        return ElementName.fromString(f.getAbsolutePath()
                .substring(this.root.getAbsolutePath().length() + 1, (f.getAbsolutePath().length() - ".class".length()))
