...
class DependencyClassVisitor extends ClassVisitor {
    ...
    public DependencyClassVisitor(final ClassVisitor visitor, final AccessVisitor typeReceiver,
    ...
    private static AccessVisitor filterOutJavaLangObject(final AccessVisitor child) {
        return new AccessVisitor() {
            ...
            @Override
            public void apply(final AccessPoint source, final AccessPoint dest, final AccessType type) {
    ...
    public void visit(final int version, final int access, final String name, final String signature,
        ...
        if (superName != null) {
            this.dependencyVisitor.apply(this.parent, AccessPoint.create(nameTransformer.transform(superName)),
                    AccessType.INHERITANCE);
        }
        for (final String each : interfaces) {
            this.dependencyVisitor.apply(this.parent, AccessPoint.create(nameTransformer.transform(each)),
                    AccessType.IMPLEMENTS);
        }

        if (signature != null) {
            final SignatureReader sr = new SignatureReader(signature);
            sr.accept(new DependencySignatureVisitor(this.parent, this.dependencyVisitor, AccessType.SIGNATURE));
        }
    }
    ...
    public FieldVisitor visitField(final int access, final String name, final String desc, final String signature,
        ...
        this.dependencyVisitor.apply(this.parent, AccessPoint.create(getElementNameForType(asmType)),
                AccessType.COMPOSED);

        if (signature != null) {
            final SignatureReader sr = new SignatureReader(signature);
            sr.accept(new DependencySignatureVisitor(this.parent, this.dependencyVisitor, AccessType.COMPOSED));
        }
    ...
    private void examineReturnType(final String desc, final AccessPoint method) {
        final org.objectweb.asm.Type returnType = org.objectweb.asm.Type.getMethodType(desc).getReturnType();
        this.dependencyVisitor.apply(method,
                AccessPoint.create(nameTransformer.transform(getElementNameForType(returnType).asInternalName())),
                AccessType.SIGNATURE);
    }

    private void examineExceptions(final String[] exceptions, final AccessPoint method) {
        if (exceptions != null) {
            for (final String each : exceptions) {
                this.dependencyVisitor.apply(method, AccessPoint.create(nameTransformer.transform(each)),
                        AccessType.SIGNATURE);
            }
        }
    }
    ...
    private void examineParameters(final String desc, final AccessPoint method) {
        final org.objectweb.asm.Type[] params = org.objectweb.asm.Type.getArgumentTypes(desc);

        for (final Type each : params) {
            this.dependencyVisitor.apply(method,
                    AccessPoint.create(nameTransformer.transform(getElementNameForType(each).asInternalName())),
                    AccessType.SIGNATURE);
        }
    }
