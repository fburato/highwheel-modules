...
class StreamUtilSest extends AnyWordSpec with Matchers with MockitoSugar with OneInstancePerTest {
  ...
  "should copy contents of stream to other" in {
    val expected = RandomUtils.nextBytes(10 * 1024 + 17)
    val actualStream = StreamUtilS.copyStream(new ByteArrayInputStream(Array.copyAs(expected, expected.length)))
