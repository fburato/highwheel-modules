...
public class StrictAnalyser {
    ...
    public static AnalyserModel.StrictAnalysisResult analyseStrict(Definition definition, AnalysisState analysisState) {
        ...
        final List<AnalyserModel.DependencyViolation> dependencyViolations = getDependencyViolations(
                specTransitiveClosure.diffPath(actualTransitiveClosure).get(), analysisState.other,
                analysisState.actualTrackingGraph);
        final List<AnalyserModel.NoStrictDependencyViolation> noStrictDependencyViolations = getNoDirectDependecyViolations(
    ...
    private static List<AnalyserModel.DependencyViolation> getDependencyViolations(
        ...
        final List<AnalyserModel.DependencyViolation> dependencyViolations = new ArrayList<>(differences.size());
        for (ModuleGraphTransitiveClosure.PathDifference difference : differences) {
            if (!difference.source.equals(other) && !difference.dest.equals(other)) {
                dependencyViolations.add(new AnalyserModel.DependencyViolation(difference.source.name,
    ...
    private static List<AnalyserModel.NoStrictDependencyViolation> getNoDirectDependecyViolations(
        ...
        final List<AnalyserModel.NoStrictDependencyViolation> noStrictDependencyViolations = new ArrayList<>();
        for (NoStrictDependency rule : rules) {
            if (!rule.source.equals(other) && !rule.dest.equals(other)
                ...
                noStrictDependencyViolations
                        .add(new AnalyserModel.NoStrictDependencyViolation(rule.source.name, rule.dest.name));
            }
        }
