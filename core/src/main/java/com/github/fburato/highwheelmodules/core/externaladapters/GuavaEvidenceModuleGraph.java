...
public class GuavaEvidenceModuleGraph implements ModuleGraph<EvidenceModuleDependency> {
    ...
    public Optional<EvidenceModuleDependency> findDependency(HWModule vertex1, HWModule vertex2) {
        return graph.findDependency(vertex1, vertex2).map((e) -> new EvidenceModuleDependency(e.source, e.dest,
                new ArrayList<>(e.getSources()).get(0), new ArrayList<>(e.getDestinations()).get(0)));
    }
    ...
    public void addDependency(EvidenceModuleDependency dependency) {
        if (graph.modules().containsAll(Arrays.asList(dependency.destModule, dependency.sourceModule))) {
            TrackingModuleDependency trackingModuleDependency = new TrackingModuleDependency(dependency.sourceModule,
                    dependency.destModule);
            final Optional<TrackingModuleDependency> dependencyOptional = graph.findDependency(dependency.sourceModule,
                    dependency.destModule);
            if (evidenceLimit.map(
                    limit -> dependencyOptional.map(TrackingModuleDependency::getEvidenceCounter).orElse(0) < limit)
                    .orElse(true)) {
