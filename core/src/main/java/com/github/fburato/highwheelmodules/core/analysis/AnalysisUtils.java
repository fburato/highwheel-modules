...
public class AnalysisUtils {
    ...
    public static List<List<Pair<String, String>>> getEvidence(ModuleGraph<TrackingModuleDependency> trackingGraph,
            HWModule source, List<HWModule> path) {
        ...
        for (int i = 0; i < completePath.size() - 1; ++i) {
            ...
            final Optional<TrackingModuleDependency> maybeDep = trackingGraph.findDependency(current, next);
            final List<Pair<String, String>> partial = new ArrayList<>();
            maybeDep.ifPresent(dependency -> dependency.getSources()
                    .forEach((apSource) -> dependency.getDestinationsFromSource(apSource)
                            .forEach((apDest) -> partial.add(Pair.make(apSource.toString(), apDest.toString())))));
            result.add(partial);
        }
        return result;
    }
    ...
    public static List<AnalyserModel.Metrics> getMetrics(ModuleMetrics moduleMetrics, Collection<HWModule> modules,
