...
private[analysis] object LooseAnalyser extends Analyser {
  override def analyse(state: AnalysisState): AnalysisResult = {
    ...
    val absentDependencies =
      getAbsentDependencies(actualTransitiveClosure, state.dependencies, state.other)
    val undesiredDependencies = getUndesiredDependencies(
    ...
    AnalysisResult(
      ...
      absentDependencies,
      getMetrics(state.actualGraph, state.modules, state.actualGraph, state.other)
    )
  }

  private def getAbsentDependencies(
    ...
    dependencies
      .filter(d =>
        d.source != other && d.dest != other && !transitiveClosure.isReachable(d.source, d.dest)
      )
      .map(d => ModuleConnectionViolation(d.source.name, d.dest.name))
  }

  private def getUndesiredDependencies(
    transitiveClosure: ModuleGraphTransitiveClosure,
      ...
      .map(d =>
        EvidenceBackedViolation(
          ...
          getEvidence(
            trackingGraph,
            d.source,
            transitiveClosure.minimumDistancePath(d.source, d.dest)
          )
        )
      )
  }
