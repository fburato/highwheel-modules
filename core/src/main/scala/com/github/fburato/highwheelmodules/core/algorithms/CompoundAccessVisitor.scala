...
class CompoundAccessVisitor(private val accessVisitors: Seq[AccessVisitor]) extends AccessVisitor {
  ...
  override def newAccessPoint(ap: AccessPoint): Unit =
    accessVisitors.foreach(av => av.newAccessPoint(ap))

  override def newEntryPoint(clazz: ElementName): Unit =
    accessVisitors.foreach(av => av.newEntryPoint(clazz))

  override def apply(source: AccessPoint, dest: AccessPoint, accessType: AccessType): Unit =
    accessVisitors.foreach(av => av.apply(source, dest, accessType))
}

object CompoundAccessVisitor {
  def apply(accessVisitors: Seq[AccessVisitor]): CompoundAccessVisitor = new CompoundAccessVisitor(
    accessVisitors
  )
}
