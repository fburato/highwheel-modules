...
class HwmCompilerTest extends AnyWordSpec with Matchers with MockitoSugar with OneInstancePerTest {
  ...
  "compile minimal specification" in {
    val result = compile(baseDefinition.copy(moduleDefinitions = List(), rules = List()))
  ...
  "fail if module regular expression fails to parse" in {
    HwmCompiler
      .compile(
        baseDefinition
          .copy(moduleDefinitions = List(ModuleDefinition("name", List("invalidregex["))))
      )
  ...
  "fail if module name appears twice" in {
    HwmCompiler
      .compile(
        baseDefinition.copy(moduleDefinitions =
          List(ModuleDefinition("name", List("name")), ModuleDefinition("name", List("name1")))
        )
      )
      .isLeft shouldBe true
  }
  ...
  "add prefix to all modules regexes" in {
    val result = compile(
      baseDefinition.copy(
        ...
        rules = List(ChainDependencyRule(List("Foo", "Bar")))
      )
    )
  ...
  "fail if rules refer to undefined modules" in {
    HwmCompiler
      .compile(
        baseDefinition.copy(
          moduleDefinitions = List(ModuleDefinition("name", List("name"))),
          rules = List(ChainDependencyRule(List("name", "NOT_NAME")))
        )
      )
      .isLeft shouldBe true
  }

  private val definitionWithModules = baseDefinition.copy(moduleDefinitions =
    List(
  ...
  "return the expected modules" in {
    ...
    result.modules should contain theSameElementsAs List(CORE, COMMONS, MAIN, IO)
  }

  "convert chain dependencies in two-by-two dependencies" in {
    val result = compile(
      definitionWithModules.copy(rules = List(ChainDependencyRule(List("main", "core", "commons"))))
    )
  ...
  "convert one to many dependency in two-by-two dependencies" in {
    val result = compile(
      definitionWithModules.copy(rules = List(OneToManyRule("main", List("core", "commons"))))
    )
    result.dependencies should contain theSameElementsAs List(
      DependencyS(MAIN, CORE),
  ...
  "fail to compile one to many if starting module does not exist" in {
    HwmCompiler
      .compile(
        definitionWithModules.copy(rules = List(OneToManyRule("NOT_MAIN", List("core", "commons"))))
      )
      .isLeft shouldBe true
  }
  ...
  "fail to compile one to many if ending module does not exist" in {
    HwmCompiler
      .compile(
        definitionWithModules.copy(rules = List(OneToManyRule("main", List("NOT_core", "commons"))))
      )
      .isLeft shouldBe true

    HwmCompiler
      .compile(
        definitionWithModules.copy(rules = List(OneToManyRule("main", List("core", "NOT_commons"))))
      )
      .isLeft shouldBe true
  }
  ...
  "convert many to one in two-by-two dependencies" in {
    val result = compile(
      definitionWithModules.copy(rules = List(ManyToOneRule(List("core", "commons"), "main")))
    )
    result.dependencies should contain theSameElementsAs List(
  ...
  "fail to compile many to one if starting module does not exist" in {
    HwmCompiler
      .compile(
        definitionWithModules.copy(rules = List(ManyToOneRule(List("NOT_core", "commons"), "main")))
      )
      .isLeft shouldBe true

    HwmCompiler
      .compile(
        definitionWithModules.copy(rules = List(ManyToOneRule(List("core", "NOT_commons"), "main")))
      )
      .isLeft shouldBe true
  }
  ...
  "fail to compile many to one if ending module does not exist" in {
    HwmCompiler
      .compile(
        definitionWithModules.copy(rules = List(ManyToOneRule(List("core", "commons"), "NOT_main")))
      )
      .isLeft shouldBe true
  }
  ...
  "convert NoDependentRule appropriately" in {
    val result =
      compile(definitionWithModules.copy(rules = List(NoDependentRule("main", "commons"))))
  ...
  "fail if whitelist contains malformed regex" in {
    HwmCompiler
      .compile(definitionWithModules.copy(whitelist = Some(List("whitelist", "invalidregex["))))
      .isLeft shouldBe true
  }

  "fail if blacklist contains malformed regex" in {
    HwmCompiler
      .compile(definitionWithModules.copy(blacklist = Some(List("whitelist", "invalidregex["))))
      .isLeft shouldBe true
  }
  ...
  "compile definition with whitelist, blacklist and prefix" in {
    val result = compile(
      baseDefinition.copy(
        ...
        moduleDefinitions =
          List(ModuleDefinition("Foo", List("foo.*")), ModuleDefinition("Bar", List("bar.*"))),
        rules = List(ChainDependencyRule(List("Foo", "Bar")))
      )
    )
  ...
  "set mode to strict if specification uses strict explicit as mode" in {
    val result = compile(baseDefinition.copy(mode = Some("STRICT")))

    result.mode shouldBe STRICT
  }

  "set mode to loose if specification uses loose explicit as mode" in {
    val result = compile(baseDefinition.copy(mode = Some("LOOSE")))
  ...
  "fail to compile if mode is not recognised" in {
    HwmCompiler
      .compile(definitionWithModules.copy(mode = Some("SOMETHING")))
      .isLeft shouldBe true
  }
}
