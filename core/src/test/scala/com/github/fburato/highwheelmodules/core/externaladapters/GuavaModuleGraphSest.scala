...
class GuavaModuleGraphSest extends AnyWordSpec with Matchers with OneInstancePerTest {
  private val graph: MutableNetwork[HWModuleS, ModuleDependencyS] = NetworkBuilder.directed().allowsSelfLoops(true).build()
  ...
  "addDependency" should {
    "add edge to network" in {
      testee.addModule(m1)
      testee.addModule(m2)
      testee.addDependency(ModuleDependencyS(m1, m2))
    ...
    "increase counter if dependency added" in {
      ...
      graph.edgeConnecting(m1, m2).get.count shouldBe 1
    }
    ...
    "increase counter multiple times if dependency added multiple times" in {
      ...
      graph.edgeConnecting(m1, m2).get.count shouldBe 2
    }
  }
  ...
  "fanIn of module" should {
    "be empty if module is not in graph" in {
      ...
      testee.fanInOf(m2).isDefined shouldBe false
    }
    ...
    "equal the amount of incoming edges" in {
      ...
      testee.addModule(m2)
      testee.addModule(m3)
      testee.addDependency(ModuleDependencyS(m2, m1))
      testee.addDependency(ModuleDependencyS(m3, m1))
    ...
    "ignore self references" in {
      ...
      testee.addModule(m2)
      testee.addDependency(ModuleDependencyS(m1, m2))
      testee.addDependency(ModuleDependencyS(m2, m2))
  ...
  "fanOut of module" should {
    "be empty if module is not in graph" in {
      ...
      testee.fanOutOf(m2).isDefined shouldBe false
    }
    ...
    "equal the amount of outgoing edges" in {
      ...
      testee.addModule(m3)
      testee.addDependency(ModuleDependencyS(m1, m2))
      testee.addDependency(ModuleDependencyS(m1, m3))
    ...
    "ignore self references" in {
      ...
      testee.addModule(m2)
      testee.addDependency(ModuleDependencyS(m1, m2))
      testee.addDependency(ModuleDependencyS(m1, m1))
