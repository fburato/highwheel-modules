...
class ModuleDependenciesGraphBuildingVisitorSest extends AnyWordSpec with Matchers with MockitoSugar with OneInstancePerTest {
  ...
  private val SUPER_MODULE = HWModule.make("SuperModule", "org.example.*").get
  private val CORE = HWModule.make("Core", "org.example.core.*").get
  private val IO = HWModule.make("IO", "org.example.io.*").get
  private val OTHER = HWModule.make("Other", "").get
  ...
  private def ap(source: String): AccessPoint = AccessPoint.create(ElementName.fromString(source))
  ...
  "apply" should {
    "ignore operation if source matches black list" in {
      ...
      moduleGraph.findDependency(CORE, IO).isPresent should be(false)
    }
    ...
    "not ignore operation if source and dest do not match black list and match white list" in {
      ...
      moduleGraph.findDependency(CORE, IO).isPresent should be(true)
    }
    ...
    "connect elements not matching to OTHER" in {
      ...
      moduleGraph.findDependency(OTHER, IO).isPresent should be(true)
      moduleGraph.findDependency(IO, OTHER).isPresent should be(true)
    }

    "not add self dependencies" in {
      testee.apply(ap("org.example.core.Service"), ap("org.example.core.OtherService"), null)

      moduleGraph.findDependency(CORE, CORE).isPresent should be(false)
    }

    "not add self dependency to OTHER" in {
      testee.apply(ap("NOTorg.example.core.Service"), ap("NOTorg.example.core.OtherService"), null)

      moduleGraph.findDependency(OTHER, OTHER).isPresent should be(false)
    }
    ...
    "add source and dest to more modules if multiple regexes match" in {
      ...
      moduleGraph.findDependency(CORE, IO).isPresent should be(true)
      moduleGraph.findDependency(CORE, SUPER_MODULE).isPresent should be(true)
      moduleGraph.findDependency(SUPER_MODULE, IO).isPresent should be(true)
    }
  }
}
