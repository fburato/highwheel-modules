...
class ParsersTest extends AnyWordSpec with Matchers with MockitoSugar with OneInstancePerTest {
  "prefix parser" should {
    "parse expected format" in {
      val stringLiteral = "literal"
      run(
        HwmParser.prefixParser,
        List(Keywords.Prefix, Operators.DefinedAs, StringLiteral(stringLiteral), NewLine)
      ) shouldBe Right(stringLiteral)
    }

    "fail on missing prefix" in {
      run(
        HwmParser.prefixParser,
        List(Operators.DefinedAs, StringLiteral(""), NewLine)
    ...
    "fail on missing column" in {
      run(
        HwmParser.prefixParser,
        List(Keywords.Prefix, StringLiteral(""), NewLine)
      ).isLeft shouldBe true
    }
    ...
    "fail on missing string literal" in {
      run(
        HwmParser.prefixParser,
        List(Keywords.Prefix, Operators.DefinedAs, NewLine)
      ).isLeft shouldBe true
    }
    ...
    "fail on missing new line" in {
      run(
        HwmParser.prefixParser,
        List(Keywords.Prefix, Operators.DefinedAs, StringLiteral(""))
      ).isLeft shouldBe true
    }
  }
  ...
  "whitelist parser" should {
    "parse expected format with one literal" in {
      val literal = "literal"
      run(
        HwmParser.whitelistParser,
        List(Keywords.Whitelist, Operators.DefinedAs, StringLiteral(literal), NewLine)
      ) shouldBe Right(List(literal))
    }
    ...
    "parse expected format with multiple literals" in {
      ...
      run(
        ...
        List(
          Keywords.Whitelist,
          Operators.DefinedAs,
          StringLiteral(literal1),
          Operators.Comma,
          StringLiteral(literal2),
          NewLine
        )
      ) shouldBe Right(List(literal1, literal2))
    }

    "fail on missing whitelist" in {
      run(
        HwmParser.whitelistParser,
        List(Operators.DefinedAs, StringLiteral(""), Operators.Comma, StringLiteral(""), NewLine)
      ).isLeft shouldBe true
    }
    ...
    "fail on missing column" in {
      run(
        HwmParser.whitelistParser,
        List(Keywords.Whitelist, StringLiteral(""), Operators.Comma, StringLiteral(""), NewLine)
      ).isLeft shouldBe true
    }
    ...
    "fail on missing literals" in {
      run(
        HwmParser.whitelistParser,
        List(Keywords.Whitelist, Operators.DefinedAs, NewLine)
      ).isLeft shouldBe true
    }
    ...
    "fail on missing commas" in {
      run(
        HwmParser.whitelistParser,
        List(Keywords.Whitelist, Operators.DefinedAs, StringLiteral(""), StringLiteral(""), NewLine)
      ).isLeft shouldBe true
    }
    ...
    "fail on malformed literal list" in {
      run(
        HwmParser.whitelistParser,
        List(Keywords.Whitelist, Operators.DefinedAs, StringLiteral(""), Operators.Comma, NewLine)
      ).isLeft shouldBe true
      run(
        HwmParser.whitelistParser,
        List(Keywords.Whitelist, Operators.DefinedAs, Operators.Comma, StringLiteral(""), NewLine)
      ).isLeft shouldBe true
    }
  ...
  "blacklist parser" should {
    "parse expected format with one literal" in {
      val literal = "literal"
      run(
        HwmParser.blacklistParser,
        List(Keywords.Blacklist, Operators.DefinedAs, StringLiteral(literal), NewLine)
      ) shouldBe Right(List(literal))
    }
    ...
    "fail on missing column" in {
      run(
        HwmParser.blacklistParser,
        List(Keywords.Blacklist, StringLiteral(""), Operators.Comma, StringLiteral(""), NewLine)
      ).isLeft shouldBe true
    }
    ...
    "fail on missing literals" in {
      run(
        HwmParser.blacklistParser,
        List(Keywords.Blacklist, Operators.DefinedAs, NewLine)
      ).isLeft shouldBe true
    }
    ...
    "fail on missing commas" in {
      run(
        HwmParser.blacklistParser,
        List(Keywords.Blacklist, Operators.DefinedAs, StringLiteral(""), StringLiteral(""), NewLine)
      ).isLeft shouldBe true
    }
    ...
    "fail on malformed literal list" in {
      run(
        HwmParser.blacklistParser,
        List(Keywords.Blacklist, Operators.DefinedAs, StringLiteral(""), Operators.Comma, NewLine)
      ).isLeft shouldBe true
      run(
        HwmParser.blacklistParser,
        List(Keywords.Blacklist, Operators.DefinedAs, Operators.Comma, StringLiteral(""), NewLine)
      ).isLeft shouldBe true
    }
  ...
  "mode parser" should {
    "parse mode definition" in {
      val mode = "STRICT"
      run(
        HwmParser.modeParser,
        List(Keywords.Mode, Operators.DefinedAs, Identifier(mode), NewLine)
      ) shouldBe Right(mode)
    }
    ...
    "fail on missing keyword" in {
      run(
        HwmParser.modeParser,
        List(Operators.DefinedAs, Identifier(""), NewLine)
      ).isLeft shouldBe true
    }

    "fail on missing column" in {
      run(HwmParser.modeParser, List(Keywords.Mode, Identifier(""), NewLine)).isLeft shouldBe true
    }

    "fail on missing identifier" in {
      run(
        HwmParser.modeParser,
        List(Keywords.Mode, Operators.DefinedAs, NewLine)
      ).isLeft shouldBe true
    }
    ...
    "fail on missing newline" in {
      run(
        HwmParser.modeParser,
        List(Keywords.Mode, Operators.DefinedAs, Identifier(""))
      ).isLeft shouldBe true
    }
  }
  ...
  "module definition parser" should {
    "parse module definition with one literal" in {
      ...
      val literal = "literal"
      run(
        HwmParser.moduleDefinitionParser,
        List(Identifier(identifier), Operators.Equals, StringLiteral(literal), NewLine)
    ...
    "fail on missing identifier" in {
      run(
        HwmParser.moduleDefinitionParser,
        List(Operators.Equals, StringLiteral(""), Operators.Comma, StringLiteral(""), NewLine)
      ).isLeft shouldBe true
    }
    ...
    "fail on missing column" in {
      run(
        HwmParser.moduleDefinitionParser,
        List(Identifier(""), StringLiteral(""), Operators.Comma, StringLiteral(""), NewLine)
      ).isLeft shouldBe true
    }
    ...
    "fail on missing literals" in {
      run(
        HwmParser.moduleDefinitionParser,
        List(Identifier(""), Operators.Equals, NewLine)
      ).isLeft shouldBe true
    }
    ...
    "fail on malformed literals" in {
      run(
        HwmParser.moduleDefinitionParser,
        List(Identifier(""), Operators.Equals, Operators.Comma, StringLiteral(""), NewLine)
      ).isLeft shouldBe true
      run(
        HwmParser.moduleDefinitionParser,
        List(Identifier(""), Operators.Equals, StringLiteral(""), Operators.Comma, NewLine)
      ).isLeft shouldBe true
    }
  ...
  "chain dependency rules parser" should {
    "parse multiple chained rules" in {
      ...
      val id3 = "id3"
      run(
        HwmParser.chainDependencyRuleParser,
        List(
          Identifier(id1),
          Operators.Arrow,
          Identifier(id2),
    ...
    "parse one chained rule" in {
      ...
      run(
        HwmParser.chainDependencyRuleParser,
        List(Identifier(id1), Operators.Arrow, Identifier(id2), NewLine)
    ...
    "fail on missing identifier" in {
      run(
        HwmParser.chainDependencyRuleParser,
        List(Operators.Arrow, Identifier(""), NewLine)
      ).isLeft shouldBe true
      run(
        HwmParser.chainDependencyRuleParser,
        List(Identifier(""), Operators.Arrow, NewLine)
      ).isLeft shouldBe true
      run(HwmParser.chainDependencyRuleParser, List(Identifier(""), NewLine)).isLeft shouldBe true
    }
  }
  ...
  "no dependent rule" should {
    "parse expected format" in {
      ...
      val id2 = "id2"
      run(
        HwmParser.noDependencyRuleParser,
        List(Identifier(id1), Operators.NotArrow, Identifier(id2), NewLine)
    ...
    "fail on malformed identifiers" in {
      run(
        HwmParser.noDependencyRuleParser,
        List(Operators.NotArrow, Identifier(""), NewLine)
      ).isLeft shouldBe true
      run(
        HwmParser.noDependencyRuleParser,
        List(Identifier(""), Identifier(""), NewLine)
      ).isLeft shouldBe true
      run(
        HwmParser.noDependencyRuleParser,
        List(Identifier(""), Operators.NotArrow, NewLine)
      ).isLeft shouldBe true
    }
  }
  ...
  "one to many rule" should {
    ...
    "fail on missing identifier" in {
      run(
        HwmParser.oneToManyRuleParser,
        List(Operators.Arrow, Operators.OpenParenthesis, Identifier(""), Operators.CloseParenthesis)
      ).isLeft shouldBe true

      run(
        HwmParser.oneToManyRuleParser,
        List(Identifier(""), Operators.Arrow, Operators.OpenParenthesis, Operators.CloseParenthesis)
      ).isLeft shouldBe true
    }
    ...
    "fail on missing arrow" in {
      run(
        HwmParser.oneToManyRuleParser,
        List(Identifier(""), Operators.OpenParenthesis, Identifier(""), Operators.CloseParenthesis)
      ).isLeft shouldBe true
    }
    ...
    "fail on missing open parenthesis" in {
      run(
        HwmParser.oneToManyRuleParser,
        List(Identifier(""), Operators.Arrow, Identifier(""), Operators.CloseParenthesis)
      ).isLeft shouldBe true
    }
    ...
    "fail on missing close parenthesis" in {
      run(
        HwmParser.oneToManyRuleParser,
        List(Identifier(""), Operators.Arrow, Operators.OpenParenthesis, Identifier(""))
      ).isLeft shouldBe true
    }
  }
  ...
  "many to one rule" should {
    ...
    "fail on missing identifier" in {
      run(
        HwmParser.manyToOneRuleParser,
        List(Operators.OpenParenthesis, Operators.CloseParenthesis, Operators.Arrow, Identifier(""))
      ).isLeft shouldBe true

      run(
        HwmParser.manyToOneRuleParser,
        List(Operators.OpenParenthesis, Identifier(""), Operators.CloseParenthesis, Operators.Arrow)
      ).isLeft shouldBe true
    }
    ...
    "fail on missing arrow" in {
      run(
        HwmParser.manyToOneRuleParser,
        List(Operators.OpenParenthesis, Identifier(""), Operators.CloseParenthesis, Identifier(""))
      ).isLeft shouldBe true
    }
    ...
    "fail on missing open parenthesis" in {
      run(
        HwmParser.manyToOneRuleParser,
        List(Identifier(""), Operators.CloseParenthesis, Operators.Arrow, Identifier(""))
      ).isLeft shouldBe true
    }
    ...
    "fail on missing close parenthesis" in {
      run(
        HwmParser.manyToOneRuleParser,
        List(Operators.OpenParenthesis, Identifier(""), Operators.Arrow, Identifier(""))
      ).isLeft shouldBe true
    }
  }
  ...
  "parser" should {
    val completeDefinition = Definition(
      ...
      List(
        ModuleDefinition(
          "Core",
          List("com.pitest.highwheel.core.*", "com.pitest.highwheel.core2.*")
        ),
    ...
    val resourceExpectations = List(
      (
        ...
        Definition(
          ...
          List(ChainDependencyRule(List("Main", "Internals")))
        )
      ),
      (
        "example-def.txt",
        completeDefinition.copy(prefix = None, whitelist = None, blacklist = None, mode = None)
      ),
      (
        "example-def-with-blacklist.txt",
        completeDefinition.copy(prefix = None, whitelist = None, mode = None)
      ),
      (
        "example-def-with-prefix.txt",
        completeDefinition.copy(whitelist = None, blacklist = None, mode = None)
      ),
    ...
    resourceExpectations foreach { case (resource, expected) =>
      s"parse $resource" in {
  ...
  def lex(s: String): Either[ParserException, List[HwmToken]] = HwmLexer(
