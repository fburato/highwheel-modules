...
class GuavaTrackingModuleGraphSest extends AnyWordSpec with Matchers with OneInstancePerTest {
  ...
  "addDependency" should {
    "create new edge if modules did not exist already" in {
      testee.addModule(m1)
      testee.addModule(m2)
      val dep = new TrackingModuleDependency(m1, m2)
    ...
    "merge evidences from different access points" in {
      testee.addModule(m1)
      testee.addModule(m2)
      val dep1 = new TrackingModuleDependency(m1, m2)
      dep1.addEvidence(ap1, ap2)
      val dep2 = new TrackingModuleDependency(m1, m2)
      ...
      edge.getDestinationsFromSource(ap1) should contain theSameElementsAs List(ap2)
      edge.getDestinationsFromSource(ap2) should contain theSameElementsAs List(ap3)
      edge.getDestinations should contain theSameElementsAs List(ap2, ap3)
      edge.getSources should contain theSameElementsAs List(ap1, ap2)
    }
    ...
    "not add evidences already present" in {
      ...
      val edge = graph.edgeConnecting(m1, m2).get
      edge.getDestinationsFromSource(ap1) should contain theSameElementsAs List(ap2)
      edge.getDestinations should contain theSameElementsAs List(ap2)
      edge.getSources should contain theSameElementsAs List(ap1)
    }
    ...
    "merge evidences from the same access point" in {
      ...
      val edge = graph.edgeConnecting(m1, m2).get
      edge.getDestinationsFromSource(ap1) should contain theSameElementsAs List(ap2, ap3)
      edge.getDestinations should contain theSameElementsAs List(ap2, ap3)
      edge.getSources should contain theSameElementsAs List(ap1)
    }
    ...
    "keep evidence sperated from different modules" in {
      ...
      val dep1 = new TrackingModuleDependency(m1, m2)
      dep1.addEvidence(ap1, ap2)
      val dep2 = new TrackingModuleDependency(m1, m3)
      dep2.addEvidence(ap1, ap3)
      ...
      edge1.getSources should contain theSameElementsAs List(ap1)
      edge1.getDestinations should contain theSameElementsAs List(ap2)

      val edge2 = graph.edgeConnecting(m1, m3).get
      edge2.getSources should contain theSameElementsAs List(ap1)
      edge2.getDestinations should contain theSameElementsAs List(ap3)
    }

    "not add dependency if one of the nodes does not exist" in {
      testee.addModule(m1)
      testee.addModule(m2)
      val dep = new TrackingModuleDependency(m1, m3)
      testee.addDependency(dep)
