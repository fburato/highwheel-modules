...
public interface FacadeTestUtils {
  ...
  class AccumulatorStrictAnalysisEvenSink implements AnalyserFacade.EventSink.StrictAnalysisEventSink {
    ...
    public void dependencyViolation(String sourceModule, String destModule, List<String> expectedPath, List<String> actualPath, List<List<Pair<String, String>>> evidences) {
      accumulator.add(String.format("DEPENDENCY_VIOLATION - {%s,%s,[%s],[%s],[%d]}",
          sourceModule,
          destModule,
          String.join(",", expectedPath),
          String.join(",", actualPath),
          evidences.size()
          )
      );
    }
  ...
  class AccumulatorLooseAnalysisEventSink implements AnalyserFacade.EventSink.LooseAnalysisEventSink {
    ...
    public void undesiredDependencyViolation(String sourceModule, String destModule, List<String> path, List<List<Pair<String, String>>> evidences) {
      accumulator.add(String.format("UNDESIRED_DEPENDENCY_VIOLATION - {%s,%s,[%s],[%d]}",
          sourceModule,
          destModule,
          String.join(",", path),
          evidences.size()
