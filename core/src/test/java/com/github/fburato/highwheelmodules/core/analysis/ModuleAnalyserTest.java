...
public class ModuleAnalyserTest {
    ...
    public void analyseStrictShouldProvideLimitedDependenciesIfEvidenceLimitConfigured() {
        ...
        final AnalyserModel.StrictAnalysisResult actual = testee(orgExamples, Optional.of(1))
                .analyseStrict(one(definition)).get(0).second;
        final AnalyserModel.DependencyViolation mainControllers = actual.dependencyViolations.stream()
        ...
        final AnalyserModel.DependencyViolation mainFacade = actual.dependencyViolations.stream()
        ...
        final AnalyserModel.DependencyViolation controllersFacade = actual.dependencyViolations.stream()
    ...
    public void analyseStrictShouldAnalyseMultipleDefinitionsWithOnePass() throws IOException {
        ...
        final List<Pair<Definition, AnalyserModel.StrictAnalysisResult>> results = testee(orgExamples, Optional.empty())
                .analyseStrict(Arrays.asList(definition1, definition2));
        final AnalyserModel.StrictAnalysisResult actual1 = results.stream().filter(p -> p.first == definition1)
                .findFirst().get().second;
        final AnalyserModel.StrictAnalysisResult actual2 = results.stream().filter(p -> p.first == definition2)
                .findFirst().get().second;
